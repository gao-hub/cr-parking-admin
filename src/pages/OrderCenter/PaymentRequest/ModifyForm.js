...
@connect(({ paymentRequestManage, loading }) => ({
  ...
  submitLoading: loading.effects['paymentRequestManage/modifyManage']
}))
export default class Modify extends PureComponent {
  ...
  changeVisible = visible => {
    if (!visible) {
      this.props.dispatch({
        type: 'paymentRequestManage/setModifyInfo',
        payload: {}
      })
    }
    this.setState({
  ...
  handleOk = async () => {
    const { dispatch, form, paymentRequestManage: { modifyInfoData } } = this.props;
    form.validateFieldsAndScroll(async(err, values) => {
      if (!err) {
        let res
        if (modifyInfoData.id) {
          res = await dispatch({
            type: 'paymentRequestManage/modifyManage',
            payload: {
              ...values,
              id: this.props.paymentRequestManage.modifyInfoData.id,
            }
          })
        } else {
          res = await dispatch({
            type: 'paymentRequestManage/addManage',
            payload: values
          })
        }
        if (res && res.status === 1) {
  ...
  render() {
    const { form: { getFieldDecorator }, paymentRequestManage: { modifyInfoData } } = this.props;
    const formConfig = {
      labelCol: { span: 5 },
      wrapperCol: { span: 18 },
    };
