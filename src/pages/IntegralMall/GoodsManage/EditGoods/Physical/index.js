...
@connect(({ GoodsManage, loading }) => ({
  GoodsManage,
  loading: loading.effects['GoodsManage/modifyManage'] ||
    loading.effects['GoodsManage/addManage'] || 
...
class PhysicalComponent extends PureComponent {
  ...
  async componentDidMount() {
    ...
    if (id) {
      ...
      await dispatch({
        type: 'GoodsManage/getModifyInfo',
        payload: {
          id
        }
      })
      const { modifyInfoData } = this.props.GoodsManage;
      this.setState({
  ...
  componentWillUnmount() {
    ...
    dispatch({
      type: 'GoodsManage/setModifyInfo',
      payload: {}
    })
    dispatch({
      type: 'GoodsManage/setCaculate',
      payload: {}
    })
  }
  ...
  // 输入售价,计算商品积分
  caculate = async (rules, value, callback) => {
    const { dispatch } = this.props;
    if (!value) {
      callback('请输入商品售价');
    }
    ...
    if (!rateReg.test(value)) {
      ...
      await dispatch({
        type: 'GoodsManage/caculateGoodsIntegral',
        payload: {
          otPrice: value
        }
      })
    ...
    form.validateFieldsAndScroll(async (err, values) => {
      if (!err) {
        ...
        if (res && res.status === 1) {
          ...
          router.push({
            pathname: '/integralMall/goodsManage/list'
          })
  ...
  render() {
    const { form, disabled, GoodsManage: { modifyInfoData, caculateData }, id, type, permission } = this.props;
          ...
          <Row gutter={24}>
            ...
            {/* <Col>
              <Form.Item label="商品编码">
                ...
                })(<Input placeholder="请输入商品编码" disabled={id ? disabled : false} />)}
              </Form.Item>
            </Col> */}
            <Col>
                ...
                {getFieldDecorator('costPrice', {
                  rules: [{
                    required: true, validator: (rules, value, callback) => {
                      ...
                      if (!rateReg.test(value)) {
                        callback('必须为数字，大于0，整数部分不能超过8位，小数部分不能超过2位！')
                      }
                      if(value === '0.00'){
                        callback('不能为0.00')
                      }
