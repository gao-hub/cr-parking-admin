...
@connect(({ unifiedLotteryActivityManagement }) => ({
...
export default class FilterIpts extends Component {
  formSubmit = async e => {
    await this.props.dispatch({
      type: 'unifiedLotteryActivityManagement/setSearchInfo',
  ...
  getFormValue = () => {
    ...
    if (formQueryData.startTime && formQueryData.startTime.length) {
      formQueryData.activityTimeStart = moment(formQueryData.startTime[0]).format(dateFormat);
      formQueryData.activityTimeEnd = moment(formQueryData.startTime[1]).format(dateFormat);
    }
