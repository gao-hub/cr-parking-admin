...
@connect(({ DetailsOfTheLuckyDrawOpportunity, loading }) => ({
  ...
  loading:
    ...
    loading.effects['DetailsOfTheLuckyDrawOpportunity/statusChangeManage'] ||
    loading.effects['DetailsOfTheLuckyDrawOpportunity/downloadMember'],
}))
@Form.create()
export default class FilterIpts extends Component {
  formSubmit = async e => {
    await this.props.dispatch({
      type: 'DetailsOfTheLuckyDrawOpportunity/setSearchInfo',
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'DetailsOfTheLuckyDrawOpportunity/getAllSelect',
      payload: {},
    });
  }
  ...
  render() {
    ...
    const { parentUtmTypes } = DetailsOfTheLuckyDrawOpportunity;
    const formConfig = {
      labelCol: { span: 7 },
      wrapperCol: { span: 17 },
    };
