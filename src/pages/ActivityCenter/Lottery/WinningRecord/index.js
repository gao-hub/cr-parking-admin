...
@connect(({ winningRecord, loading }) => ({
  winningRecord,
  loading:
    loading.effects['winningRecord/fetchList'] ||
    loading.effects['winningRecord/getModifyInfo'],
  exportLoading: loading.effects['winningRecord/exportFile'],
}))
export default class template extends PureComponent {
...

      {
        ...
        render: (text, record) => {
          ...
          const action = (
            <Menu>
              {
                permission.includes('chuangrong:activityRecord:update') &&(record.deliveryStatus===3 || record.deliveryStatus===4)  ? (
                  <Menu.Item  onClick={()=>this.deliveryData(record)}>
                    发货
                  </Menu.Item>
                ) : null
              }
  ...
  getList = async (currPage, pageSize) => {
    ...
    this.props.dispatch({
      type: 'winningRecord/fetchList',
      payload: {
        currPage,
        pageSize,
        ...this.props.winningRecord.searchInfo,
      },
    });
  };
  //  删除数据
  ...
  renderBtn = () => {
    ...
    return (
      <Fragment>
        {permission.includes('chuangrong:activityRecord:export') ? (
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper renderBtn={this.renderBtn}>
        {
          permission.includes('chuangrong:activityRecord:list') && <Card bordered={false}>
            <FilterIpts
