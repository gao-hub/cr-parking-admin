...
@connect(({ homeConfig, loading }) => ({
  homeConfig,
  loading:
    loading.effects['homeConfig/fetchList'] ||
    loading.effects['homeConfig/getModifyInfo'] ||
    loading.effects['homeConfig/fetchTabList'] ||
    loading.effects['homeConfig/getTabModifyInfo'],
...
export default class template extends PureComponent {
    ...
    staticColumns: [
      {
        ...
        render: record => {
          ...
          const action = (
            <Menu>
              {permission.includes('chuangrong:travelAdvert:update') ? (
              ...
              {permission.includes('chuangrong:travelAdvert:delete') ? (
  ...
  getList = (currPage, pageSize) => {
    ...
    this.props.dispatch({
      ...
      payload: {
        currPage,
        pageSize,
        ...this.props.homeConfig.searchInfo,
      },
    });
  };
  ...
  updateTabStatusHandler = async record => {
    ...
    if (confirmVal.value) {
      const res = await this.props.dispatch({
        type: 'homeConfig/updateTabStatus',
        payload: {
          id: record.id,
          status: record.status == 0 ? 1 : 0,
        },
  ...
  renderBtn = () => {
    ...
    return (
      <Fragment>
        ...
        /> */}
        {permission.includes('chuangrong:travelAdvert:add') ? (
  ...
  tabModifyHandler = async id => {
    const res = await this.props.dispatch({
      type: 'homeConfig/getTabModifyInfo',
      payload: {
        id,
      },
    });
  ...
  async tabDeleteHandler(id) {
    ...
    if (confirmVal.value) {
      const res = await this.props.dispatch({
        type: 'homeConfig/tabDeleteManage',
        payload: {
          id,
        },
      });
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (this.state.firstFlag && permission.includes('chuangrong:travelAdvert:list')) {
      this.getList(this.state.currPage, this.state.pageSize);
      this.setState({
        firstFlag: false,
      });
    }
    if (this.state.firstFlag && permission.includes('chuangrong:travelTab:list')) {
      this.getTabList(this.state.currPage, this.state.pageSize);
      this.setState({
        firstFlag: false,
      });
    }
  ...
  callback = async index => {
    ...
    if (index == 1) {
      // this.getList(this.state.currPage, this.state.pageSize);
      if (permission.includes('chuangrong:travelAdvert:list')) {
        this.getList(1, 10);
      }
    } else {
      // this.getTabList(this.state.tabCurrPage, this.state.tabPageSize);
      if (permission.includes('chuangrong:travelTab:list')) {
  ...
  render() {
    const {
      permission,
      homeConfig: { list, total, tabList, tabTotal },
    } = this.props;
    const { currPage, pageSize, tabCurrPage, tabPageSize, data, selectedRows } = this.state;
    const values = {
