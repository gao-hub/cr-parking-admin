...
@connect(({ appVersionManage, loading }) => ({
  appVersionManage,
  loading: loading.effects['appVersionManage/setSearchInfo'],
}))
export default class FilterIpts extends Component {
  formSubmit = async e => {
    await this.props.dispatch({
      type: 'appVersionManage/setSearchInfo',
  ...
  getFormValue = () => {
    ...
    if (formQueryData.updateTime && formQueryData.updateTime.length) {
      formQueryData.versionUpdateStart = moment(formQueryData.updateTime[0]).format('YYYY-MM-DD');
      formQueryData.versionUpdateEnd = moment(formQueryData.updateTime[1]).format('YYYY-MM-DD');
    }
  ...
  render() {
    const {
      ...
      searchWholeState,
      appVersionManage: { selectListJson = {} },
    } = this.props;

    const { versionUpType = [] } = selectListJson;
        ...
        <div className={tableStyles.tableListForm}>
          <Form>
            <Row gutter={24} type="flex">
              ...
              <Col {...inputConfig}>
                <FormItem label="更新方式" {...formItemConfig}>
