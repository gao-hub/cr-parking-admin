...
class ModifyForm extends PureComponent {
  ...
  submitTradePsdApi = async params => {
    ...
    let apiUrl = '';
    if (modalJson.sign === 'penalty') {
      apiUrl = 'falsifySendManage/penaltyHandOut';
    }
    if (modalJson.sign === 'return') {
      apiUrl = 'falsifySendManage/returnHandOut';
    }
    if (modalJson.status === 1) {
      apiUrl = 'falsifySendManage/reReturnHandOut';
    }
    ...
    if (res && res.status === 1) {
      ...
      if (res.data) {
        ...
        const resData = await dispatch({
          type: 'falsifySendManage/getPhone',
          payload: {},
        });
  ...
  handleSubmitSmsCode = () => {
    ...
    form.validateFieldsAndScroll(async (err, values) => {
      if (!err) {
        ...
        let apiUrl = '';
        if (modalJson.sign === 'penalty') {
          apiUrl = 'falsifySendManage/penaltyHandOutSmsCode';
        }
        if (modalJson.sign === 'return') {
          apiUrl = 'falsifySendManage/returnHandOutSmsCode';
        }
