...
class ModifyForm extends PureComponent {
    ...
    if (isShow) {
      ...
      if (res && res.status === 1) {
        if (res.data) {
          ...
          if (window.pgeditor && document.getElementById('ocx_password')) {
            document.getElementById('ocx_password').innerHTML = window.pgeditor.load();
    ...
    if (res && res.status === 1) {
      ...
      if (res.data) {
        ...
        const resData = await dispatch({
          type: 'parkingAuditManage/getPhone',
          payload: {},
        });
  ...
  submitTradePsdApi2 = async params => {
    ...
    const res = await dispatch({
      type: 'parkingAuditManage/updateSynchroSale',
      payload: {
        ...modalJson,
        ...params,
      },
  ...
  handleSubmitSmsCode = () => {
    ...
    form.validateFieldsAndScroll(async (err, values) => {
      if (!err) {
        ...
        const res = await dispatch({
          type: 'parkingAuditManage/lianlianSmsCode',
          payload: {
            ...modalJson,
            ...values,
            ...params,
    ...
    return (
      ...
      >
        <div style={{ display: isSmsCodeShow === 1 ? 'none' : 'block', textAlign: 'center' }}>
          ...
          <span height="40" id="ocx_password" />
        </div>
